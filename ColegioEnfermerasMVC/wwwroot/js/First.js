$(document).ready(function () {
    ...
    function toggleSubmitButton() {
        const submitButton = document.getElementById('btnAdd'); // Asegúrate de que este ID coincida con el botón que deseas activar/desactivar
        if (areAllFieldsValid()) {
            submitButton.disabled = false; // Habilitar el botón si todos los campos son válidos
        } else {
            submitButton.disabled = true; // Deshabilitar el botón si algún campo es inválido
        }
    }
...
function displayMedicines(page) {
    ...
    paginatedMedicines.forEach(function (medicine) {
        ...
        <tr data-medicine-id="${medicine.medicineId}">
            ...
            <td>${medicine.medicineStock || '0'}</td>
            <td>
...
function SendButton() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            alert("Botiquín y medicamentos agregados correctamente.");
